...
@section('page-heading', __('app.features'))
...
@section ('breadcrumbs')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
        ...
        <li class="breadcrumb-item text-muted">@lang('app.feature_list')</li>
    </ul>
    <!--end::Breadcrumb-->
@stop
...
@section('content')
    @section('actions')
        <a href="{{ route('features.create') }}" class="btn btn-sm btn-primary">
            ...
            @lang('app.add_feature')
        </a>
    @endsection
    @include('partials.messages')

    {!! Form::open(['route' => 'features.save', 'class' => 'mb-4']) !!}
                    ...
                    <table id="kt_datatable_example_1" class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="kt_table_users">
                        ...
                        @if (count($features))
                            @foreach ($features as $feature)
                                <tr>
                                    <td>{{ $feature->display_name ?: $feature->name }}</td>
                                        ...
                                        <td class="text-center">
                                            <div class="form-check form-check-sm form-check-custom form-check-solid me-5 me-lg-20">
                                                {{ html()->checkbox()
                                                         ...
                                                         ->id("cb-{$plan->id}-{$feature->id}")
                                                         ->name("plans[{$plan->id}][]")
                                                         ->checked($plan->hasFeature($feature->name))
                                                         ->value($feature->id)
                                                }}
                                                <label class="custom-control-label d-inline"
                                                       for="cb-{{ $plan->id }}-{{ $feature->id }}">
                                    ...
                                    <td class="text-end">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                            <div class="menu-item px-3">
                                                <a href="{{ route('features.edit', $feature) }}" class="menu-link px-3">
                                            ...
                                            @if ($feature->removable)
                                                <div class="menu-item px-3">
                                                    <a href="{{ route('features.destroy', $feature) }}" class="menu-link px-3 delete_confirm"
        ...
        $('.delete_confirm').click(function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.setAttribute("id", "deletePlanForm");
