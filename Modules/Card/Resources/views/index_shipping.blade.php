...
@section('page-title', __('app.shippings'))
@section('page-heading', __('app.shippings'))
...
<h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">@lang('app.card')</h1>
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
        <li class="breadcrumb-item text-muted">
            <a href="" class="text-muted text-hover-primary">@lang('app.shippings_list')</a>
        </li>
    </ul>
@stop
@section('actions')
    <a href="{{ route('shippings.create') }}" class="btn btn-sm btn-primary">
...
@include('partials.messages')
    <x-card-content>
        ...
        <x-card-body>
            <x-datatable-html>
                <td>{{__("app.is_main")}}</td>
                <td>{{__("app.country")}}</td>
                <td>{{__("app.state")}}</td>
                <td>{{__("app.zip_code")}}</td>
